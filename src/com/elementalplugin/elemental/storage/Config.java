...
public class Config {
    ...
    private static Config create(String path) {
        ...
        return new Config(new File(Elemental.getFolder(), "/configuration/" + path)).reload();
    }
        ...
        for (Field field : object.getClass().getDeclaredFields()) {
            if (field.isAnnotationPresent(Configure.class)) {
                ...
                try {
                    ...
                    e.printStackTrace();
                    Elemental.plugin().getLogger().warning("Unable to set config value of " + field.getName() + " for " + object.getFileName());
                }
            }
        }
